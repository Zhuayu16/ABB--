...
class ABBParamDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("ABB机械臂专业参数配置")
    ...
    def _create_basic_tab(self):
        ...
        robot_layout.addRow("机器人型号:", self.robot_model_combo)
        
        self.payload_spin = QDoubleSpinBox()
        self.payload_spin.setRange(0.1, 500.0)
        self.payload_spin.setValue(10.0)
        self.payload_spin.setSuffix(" kg")
        robot_layout.addRow("负载重量:", self.payload_spin)
        ...
        self.tool_name_edit = QLineEdit("tool0")
        coord_layout.addRow("工具名称:", self.tool_name_edit)
        
        self.workobj_name_edit = QLineEdit("wobj0")
        coord_layout.addRow("工件坐标系:", self.workobj_name_edit)
        ...
        speed_layout.addRow("运动速度:", self.speed_spin)
        
        self.zone_spin = QDoubleSpinBox()
        self.zone_spin.setRange(0.1, 100)
        self.zone_spin.setValue(10)
        self.zone_spin.setSuffix(" mm")
        speed_layout.addRow("区段精度:", self.zone_spin)
        
        self.acceleration_spin = QDoubleSpinBox()
        self.acceleration_spin.setRange(0.1, 100)
        self.acceleration_spin.setValue(50)
        self.acceleration_spin.setSuffix(" %")
        speed_layout.addRow("加速度:", self.acceleration_spin)
        
        layout.addWidget(speed_group)
        
        layout.addStretch()
        self.tab_widget.addTab(tab, "基础参数")
    ...
    def _create_tcp_tab(self):
        ...
        # TCP位置调整
        tcp_pos_group = QGroupBox("TCP位置调整")
        tcp_pos_layout = QFormLayout(tcp_pos_group)
        
        self.tcp_x_spin = QDoubleSpinBox()
        self.tcp_x_spin.setRange(-500, 500)
        self.tcp_x_spin.setValue(0)
        self.tcp_x_spin.setSuffix(" mm")
        tcp_pos_layout.addRow("X偏移:", self.tcp_x_spin)
        
        self.tcp_y_spin = QDoubleSpinBox()
        self.tcp_y_spin.setRange(-500, 500)
        self.tcp_y_spin.setValue(0)
        self.tcp_y_spin.setSuffix(" mm")
        tcp_pos_layout.addRow("Y偏移:", self.tcp_y_spin)
        
        self.tcp_z_spin = QDoubleSpinBox()
        self.tcp_z_spin.setRange(-500, 500)
        self.tcp_z_spin.setValue(0)
        self.tcp_z_spin.setSuffix(" mm")
        tcp_pos_layout.addRow("Z偏移:", self.tcp_z_spin)
        
        layout.addWidget(tcp_pos_group)
        
        # TCP姿态调整
        tcp_rot_group = QGroupBox("TCP姿态调整")
        tcp_rot_layout = QFormLayout(tcp_rot_group)
        
        self.tcp_rx_spin = QDoubleSpinBox()
        self.tcp_rx_spin.setRange(-180, 180)
        self.tcp_rx_spin.setValue(0)
        self.tcp_rx_spin.setSuffix(" °")
        tcp_rot_layout.addRow("RX旋转:", self.tcp_rx_spin)
        
        self.tcp_ry_spin = QDoubleSpinBox()
        self.tcp_ry_spin.setRange(-180, 180)
        self.tcp_ry_spin.setValue(0)
        self.tcp_ry_spin.setSuffix(" °")
        tcp_rot_layout.addRow("RY旋转:", self.tcp_ry_spin)
        
        self.tcp_rz_spin = QDoubleSpinBox()
        self.tcp_rz_spin.setRange(-180, 180)
        self.tcp_rz_spin.setValue(0)
        self.tcp_rz_spin.setSuffix(" °")
        tcp_rot_layout.addRow("RZ旋转:", self.tcp_rz_spin)
        
        layout.addWidget(tcp_rot_group)
        
        # TCP校准
        tcp_cal_group = QGroupBox("TCP校准")
        tcp_cal_layout = QVBoxLayout(tcp_cal_group)
        
        cal_btn_layout = QHBoxLayout()
        self.tcp_4point_btn = QPushButton("4点校准")
        self.tcp_6point_btn = QPushButton("6点校准")
        self.tcp_manual_btn = QPushButton("手动校准")
        
        cal_btn_layout.addWidget(self.tcp_4point_btn)
        cal_btn_layout.addWidget(self.tcp_6point_btn)
        cal_btn_layout.addWidget(self.tcp_manual_btn)
        
        tcp_cal_layout.addLayout(cal_btn_layout)
        
        self.tcp_accuracy_label = QLabel("TCP精度: 未校准")
        self.tcp_accuracy_label.setStyleSheet("color: #FF4500; font-weight: bold;")
        tcp_cal_layout.addWidget(self.tcp_accuracy_label)
        
        layout.addWidget(tcp_cal_group)
        
        layout.addStretch()
        self.tab_widget.addTab(tab, "TCP调整")
    ...
    def _create_motion_tab(self):
        ...
        # 运动模式
        motion_mode_group = QGroupBox("运动模式")
        motion_mode_layout = QVBoxLayout(motion_mode_group)
        
        self.motion_mode_combo = QComboBox()
        self.motion_mode_combo.addItems(["关节运动", "直线运动", "圆弧运动", "螺旋运动"])
        motion_mode_layout.addWidget(QLabel("运动模式:"))
        motion_mode_layout.addWidget(self.motion_mode_combo)
        
        layout.addWidget(motion_mode_group)
        
        # 路径规划
        path_group = QGroupBox("路径规划")
        path_layout = QFormLayout(path_group)
        
        self.path_smoothing_check = QCheckBox("启用路径平滑")
        self.path_smoothing_check.setChecked(True)
        path_layout.addRow(self.path_smoothing_check)
        
        self.collision_check = QCheckBox("启用碰撞检测")
        self.collision_check.setChecked(True)
        path_layout.addRow(self.collision_check)
        
        self.singularity_check = QCheckBox("避免奇异点")
        self.singularity_check.setChecked(True)
        path_layout.addRow(self.singularity_check)
        
        self.joint_limit_check = QCheckBox("关节限位检查")
        self.joint_limit_check.setChecked(True)
        path_layout.addRow(self.joint_limit_check)
        
        layout.addWidget(path_group)
        
        # 运动参数
        motion_param_group = QGroupBox("运动参数")
        motion_param_layout = QFormLayout(motion_param_group)
        
        self.max_joint_speed_spin = QDoubleSpinBox()
        self.max_joint_speed_spin.setRange(1, 500)
        self.max_joint_speed_spin.setValue(250)
        self.max_joint_speed_spin.setSuffix(" °/s")
        motion_param_layout.addRow("最大关节速度:", self.max_joint_speed_spin)
        
        self.max_tcp_speed_spin = QDoubleSpinBox()
        self.max_tcp_speed_spin.setRange(1, 2000)
        self.max_tcp_speed_spin.setValue(1000)
        self.max_tcp_speed_spin.setSuffix(" mm/s")
        motion_param_layout.addRow("最大TCP速度:", self.max_tcp_speed_spin)
        
        self.acceleration_time_spin = QDoubleSpinBox()
        self.acceleration_time_spin.setRange(0.1, 10)
        self.acceleration_time_spin.setValue(1.0)
        self.acceleration_time_spin.setSuffix(" s")
        motion_param_layout.addRow("加速时间:", self.acceleration_time_spin)
        
        layout.addWidget(motion_param_group)
        
        layout.addStretch()
        self.tab_widget.addTab(tab, "运动控制")
    
    def _create_welding_tab(self):
        """创建焊接参数标签页"""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        
        # 焊接工艺参数
        welding_group = QGroupBox("焊接工艺参数")
        welding_layout = QFormLayout(welding_group)
        
        self.welding_current_spin = QDoubleSpinBox()
        self.welding_current_spin.setRange(50, 500)
        self.welding_current_spin.setValue(200)
        self.welding_current_spin.setSuffix(" A")
        welding_layout.addRow("焊接电流:", self.welding_current_spin)
        
        self.welding_voltage_spin = QDoubleSpinBox()
        self.welding_voltage_spin.setRange(10, 50)
        self.welding_voltage_spin.setValue(25)
        self.welding_voltage_spin.setSuffix(" V")
        welding_layout.addRow("焊接电压:", self.welding_voltage_spin)
        
        self.wire_feed_speed_spin = QDoubleSpinBox()
        self.wire_feed_speed_spin.setRange(1, 50)
        self.wire_feed_speed_spin.setValue(10)
        self.wire_feed_speed_spin.setSuffix(" m/min")
        welding_layout.addRow("送丝速度:", self.wire_feed_speed_spin)
        
        self.gas_flow_spin = QDoubleSpinBox()
        self.gas_flow_spin.setRange(5, 50)
        self.gas_flow_spin.setValue(20)
        self.gas_flow_spin.setSuffix(" L/min")
        welding_layout.addRow("保护气流量:", self.gas_flow_spin)
        
        layout.addWidget(welding_group)
        
        # 焊接路径参数
        path_group = QGroupBox("焊接路径参数")
        path_layout = QFormLayout(path_group)
        
        self.weld_speed_spin = QDoubleSpinBox()
        self.weld_speed_spin.setRange(1, 100)
        self.weld_speed_spin.setValue(20)
        self.weld_speed_spin.setSuffix(" mm/s")
        path_layout.addRow("焊接速度:", self.weld_speed_spin)
        
        self.weld_width_spin = QDoubleSpinBox()
        self.weld_width_spin.setRange(1, 20)
        self.weld_width_spin.setValue(5)
        self.weld_width_spin.setSuffix(" mm")
        path_layout.addRow("焊缝宽度:", self.weld_width_spin)
        
        self.weld_height_spin = QDoubleSpinBox()
        self.weld_height_spin.setRange(0.5, 10)
        self.weld_height_spin.setValue(2)
        self.weld_height_spin.setSuffix(" mm")
        path_layout.addRow("焊缝高度:", self.weld_height_spin)
        
        self.weaving_check = QCheckBox("启用摆动焊接")
        self.weaving_check.setChecked(False)
        path_layout.addRow(self.weaving_check)
        
        self.weaving_width_spin = QDoubleSpinBox()
        self.weaving_width_spin.setRange(1, 20)
        self.weaving_width_spin.setValue(5)
        self.weaving_width_spin.setSuffix(" mm")
        self.weaving_width_spin.setEnabled(False)
        path_layout.addRow("摆动宽度:", self.weaving_width_spin)
        
        # 连接摆动参数
        self.weaving_check.toggled.connect(self.weaving_width_spin.setEnabled)
        
        layout.addWidget(path_group)
        
        # 焊接质量监控
        quality_group = QGroupBox("焊接质量监控")
        quality_layout = QFormLayout(quality_group)
        
        self.arc_monitoring_check = QCheckBox("电弧监控")
        self.arc_monitoring_check.setChecked(True)
        quality_layout.addRow(self.arc_monitoring_check)
        
        self.temperature_monitoring_check = QCheckBox("温度监控")
        self.temperature_monitoring_check.setChecked(True)
        quality_layout.addRow(self.temperature_monitoring_check)
        
        self.defect_detection_check = QCheckBox("缺陷检测")
        self.defect_detection_check.setChecked(True)
        quality_layout.addRow(self.defect_detection_check)
        
        layout.addWidget(quality_group)
        
        layout.addStretch()
        self.tab_widget.addTab(tab, "焊接参数")
    ...
    def _create_safety_tab(self):
        ...
        # 安全区域
        safety_zone_group = QGroupBox("安全区域设置")
        safety_zone_layout = QFormLayout(safety_zone_group)
        
        self.safety_zone_enable_check = QCheckBox("启用安全区域")
        self.safety_zone_enable_check.setChecked(True)
        safety_zone_layout.addRow(self.safety_zone_enable_check)
        
        self.safety_zone_x_min_spin = QDoubleSpinBox()
        self.safety_zone_x_min_spin.setRange(-2000, 2000)
        self.safety_zone_x_min_spin.setValue(-1000)
        self.safety_zone_x_min_spin.setSuffix(" mm")
        safety_zone_layout.addRow("X最小值:", self.safety_zone_x_min_spin)
        
        self.safety_zone_x_max_spin = QDoubleSpinBox()
        self.safety_zone_x_max_spin.setRange(-2000, 2000)
        self.safety_zone_x_max_spin.setValue(1000)
        self.safety_zone_x_max_spin.setSuffix(" mm")
        safety_zone_layout.addRow("X最大值:", self.safety_zone_x_max_spin)
        
        self.safety_zone_y_min_spin = QDoubleSpinBox()
        self.safety_zone_y_min_spin.setRange(-2000, 2000)
        self.safety_zone_y_min_spin.setValue(-1000)
        self.safety_zone_y_min_spin.setSuffix(" mm")
        safety_zone_layout.addRow("Y最小值:", self.safety_zone_y_min_spin)
        
        self.safety_zone_y_max_spin = QDoubleSpinBox()
        self.safety_zone_y_max_spin.setRange(-2000, 2000)
        self.safety_zone_y_max_spin.setValue(1000)
        self.safety_zone_y_max_spin.setSuffix(" mm")
        safety_zone_layout.addRow("Y最大值:", self.safety_zone_y_max_spin)
        
        self.safety_zone_z_min_spin = QDoubleSpinBox()
        self.safety_zone_z_min_spin.setRange(-2000, 2000)
        self.safety_zone_z_min_spin.setValue(-500)
        self.safety_zone_z_min_spin.setSuffix(" mm")
        safety_zone_layout.addRow("Z最小值:", self.safety_zone_z_min_spin)
        
        self.safety_zone_z_max_spin = QDoubleSpinBox()
        self.safety_zone_z_max_spin.setRange(-2000, 2000)
        self.safety_zone_z_max_spin.setValue(1500)
        self.safety_zone_z_max_spin.setSuffix(" mm")
        safety_zone_layout.addRow("Z最大值:", self.safety_zone_z_max_spin)
        
        layout.addWidget(safety_zone_group)
        
        # 安全功能
        safety_func_group = QGroupBox("安全功能")
        safety_func_layout = QFormLayout(safety_func_group)
        
        self.emergency_stop_check = QCheckBox("急停功能")
        self.emergency_stop_check.setChecked(True)
        safety_func_layout.addRow(self.emergency_stop_check)
        
        self.collision_detection_check = QCheckBox("碰撞检测")
        self.collision_detection_check.setChecked(True)
        safety_func_layout.addRow(self.collision_detection_check)
        
        self.force_limiting_check = QCheckBox("力限制")
        self.force_limiting_check.setChecked(True)
        safety_func_layout.addRow(self.force_limiting_check)
        
        self.speed_limiting_check = QCheckBox("速度限制")
        self.speed_limiting_check.setChecked(True)
        safety_func_layout.addRow(self.speed_limiting_check)
        
        self.max_force_spin = QDoubleSpinBox()
        self.max_force_spin.setRange(10, 1000)
        self.max_force_spin.setValue(100)
        self.max_force_spin.setSuffix(" N")
        safety_func_layout.addRow("最大力限制:", self.max_force_spin)
        
        layout.addWidget(safety_func_group)
        
        layout.addStretch()
        self.tab_widget.addTab(tab, "安全参数")
    ...
    def _create_communication_tab(self):
        ...
        # 网络通信
        network_group = QGroupBox("网络通信")
        network_layout = QFormLayout(network_group)
        
        self.robot_ip_edit = QLineEdit("192.168.1.100")
        network_layout.addRow("机器人IP:", self.robot_ip_edit)
        
        self.robot_port_spin = QSpinBox()
        self.robot_port_spin.setRange(1, 65535)
        self.robot_port_spin.setValue(5000)
        network_layout.addRow("通信端口:", self.robot_port_spin)
        ...
        self.heartbeat_check = QCheckBox("心跳检测")
        self.heartbeat_check.setChecked(True)
        network_layout.addRow(self.heartbeat_check)
        ...
        self.data_sampling_rate_spin = QSpinBox()
        self.data_sampling_rate_spin.setRange(1, 1000)
        self.data_sampling_rate_spin.setValue(100)
        self.data_sampling_rate_spin.setSuffix(" Hz")
        data_layout.addRow("采样频率:", self.data_sampling_rate_spin)
        
        self.data_logging_check = QCheckBox("数据记录")
        self.data_logging_check.setChecked(True)
        data_layout.addRow(self.data_logging_check)
        
        self.real_time_monitoring_check = QCheckBox("实时监控")
        self.real_time_monitoring_check.setChecked(True)
        data_layout.addRow(self.real_time_monitoring_check)
        
        layout.addWidget(data_group)
        
        # 外部设备
        device_group = QGroupBox("外部设备")
        device_layout = QFormLayout(device_group)
        
        self.vision_system_check = QCheckBox("视觉系统")
        self.vision_system_check.setChecked(False)
        device_layout.addRow(self.vision_system_check)
        
        self.force_sensor_check = QCheckBox("力传感器")
        self.force_sensor_check.setChecked(False)
        device_layout.addRow(self.force_sensor_check)
        
        self.laser_sensor_check = QCheckBox("激光传感器")
        self.laser_sensor_check.setChecked(False)
        device_layout.addRow(self.laser_sensor_check)
        
        self.welding_power_check = QCheckBox("焊接电源")
        self.welding_power_check.setChecked(True)
        device_layout.addRow(self.welding_power_check)
        
        layout.addWidget(device_group)
        
        layout.addStretch()
        self.tab_widget.addTab(tab, "通信参数")
    
    def load_preset(self):
        """加载预设参数"""
        from PyQt5.QtWidgets import QFileDialog
        file_path, _ = QFileDialog.getOpenFileName(
            self, "加载预设文件", "", "JSON文件 (*.json);;所有文件 (*.*)"
        )
        if file_path:
            try:
                import json
                ...
                QMessageBox.information(self, "成功", "预设参数加载成功！")
            except Exception as e:
    ...
    def save_preset(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "保存预设文件", "abb_preset.json", "JSON文件 (*.json);;所有文件 (*.*)"
        )
        if file_path:
            try:
                import json
                preset_data = self.get_params()
                with open(file_path, 'w', encoding='utf-8') as f:
                    json.dump(preset_data, f, ensure_ascii=False, indent=2)
                QMessageBox.information(self, "成功", "预设参数保存成功！")
            except Exception as e:
    ...
    def reset_to_default(self):
        """重置为默认参数"""
        # 基础参数
        self.robot_model_combo.setCurrentText("IRB 1200-5/0.9")
        self.payload_spin.setValue(10.0)
        self.tool_name_edit.setText("tool0")
        self.workobj_name_edit.setText("wobj0")
        self.speed_spin.setValue(100)
        self.zone_spin.setValue(10)
        self.acceleration_spin.setValue(50)
        ...
        # 运动参数
        self.motion_mode_combo.setCurrentText("直线运动")
        self.path_smoothing_check.setChecked(True)
        self.collision_check.setChecked(True)
        self.singularity_check.setChecked(True)
        self.joint_limit_check.setChecked(True)
        ...
        # 通信参数
        self.robot_ip_edit.setText("192.168.1.100")
        self.robot_port_spin.setValue(5000)
        self.timeout_spin.setValue(30)
        self.heartbeat_check.setChecked(True)
        self.data_sampling_rate_spin.setValue(100)
    ...
    def _apply_preset(self, preset_data):
        """应用预设参数"""
        # 这里可以根据预设数据的结构来设置各个参数
        # 为了简化，这里只设置一些基本参数
        if 'robot_model' in preset_data:
            self.robot_model_combo.setCurrentText(preset_data['robot_model'])
        if 'payload' in preset_data:
            self.payload_spin.setValue(preset_data['payload'])
        # ... 其他参数的设置
    ...
    def get_params(self):
        ...
        return {
            ...
            'workobj_name': self.workobj_name_edit.text(),
            'speed': self.speed_spin.value(),
            'zone': self.zone_spin.value(),
            'acceleration': self.acceleration_spin.value(),
            
            # TCP参数
            'tcp_x': self.tcp_x_spin.value(),
            'tcp_y': self.tcp_y_spin.value(),
            'tcp_z': self.tcp_z_spin.value(),
            'tcp_rx': self.tcp_rx_spin.value(),
            'tcp_ry': self.tcp_ry_spin.value(),
            'tcp_rz': self.tcp_rz_spin.value(),
            
            # 运动参数
            'motion_mode': self.motion_mode_combo.currentText(),
            'path_smoothing': self.path_smoothing_check.isChecked(),
            'collision_detection': self.collision_check.isChecked(),
            'singularity_avoidance': self.singularity_check.isChecked(),
            'joint_limit_check': self.joint_limit_check.isChecked(),
            'max_joint_speed': self.max_joint_speed_spin.value(),
            'max_tcp_speed': self.max_tcp_speed_spin.value(),
            'acceleration_time': self.acceleration_time_spin.value(),
            
            # 焊接参数
            'welding_current': self.welding_current_spin.value(),
            'welding_voltage': self.welding_voltage_spin.value(),
            'wire_feed_speed': self.wire_feed_speed_spin.value(),
            'gas_flow': self.gas_flow_spin.value(),
            'weld_speed': self.weld_speed_spin.value(),
            'weld_width': self.weld_width_spin.value(),
            'weld_height': self.weld_height_spin.value(),
            'weaving_enabled': self.weaving_check.isChecked(),
            'weaving_width': self.weaving_width_spin.value(),
            'arc_monitoring': self.arc_monitoring_check.isChecked(),
            'temperature_monitoring': self.temperature_monitoring_check.isChecked(),
            'defect_detection': self.defect_detection_check.isChecked(),
            
            # 安全参数
            'safety_zone_enabled': self.safety_zone_enable_check.isChecked(),
            'safety_zone_x_min': self.safety_zone_x_min_spin.value(),
            'safety_zone_x_max': self.safety_zone_x_max_spin.value(),
            'safety_zone_y_min': self.safety_zone_y_min_spin.value(),
            'safety_zone_y_max': self.safety_zone_y_max_spin.value(),
            'safety_zone_z_min': self.safety_zone_z_min_spin.value(),
            'safety_zone_z_max': self.safety_zone_z_max_spin.value(),
            'emergency_stop': self.emergency_stop_check.isChecked(),
            'collision_detection': self.collision_detection_check.isChecked(),
            'force_limiting': self.force_limiting_check.isChecked(),
            'speed_limiting': self.speed_limiting_check.isChecked(),
            ...
            'heartbeat': self.heartbeat_check.isChecked(),
            'data_sampling_rate': self.data_sampling_rate_spin.value(),
            'data_logging': self.data_logging_check.isChecked(),
            'real_time_monitoring': self.real_time_monitoring_check.isChecked(),
            'vision_system': self.vision_system_check.isChecked(),
            'force_sensor': self.force_sensor_check.isChecked(),
            'laser_sensor': self.laser_sensor_check.isChecked(),
            'welding_power': self.welding_power_check.isChecked(),
            
            # 兼容旧版本的参数格式
            'tcp': f"[{self.tcp_x_spin.value()},{self.tcp_y_spin.value()},{self.tcp_z_spin.value()},{self.tcp_rx_spin.value()},{self.tcp_ry_spin.value()},{self.tcp_rz_spin.value()}]",
            'wobj': "[0,0,0,0,0,0]"
        }
...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        title.setStyleSheet("font-size: 25px; font-family: Misans, Arial, sans-serif; font-weight: bold; color: #FF9900;")
...
class StressAnalysisDialog(QDialog):
    """受力分析参数设置对话框"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("受力分析参数设置")
    ...
    def init_ui(self):
        ...
        material_layout.addWidget(QLabel("材料类型:"), 0, 0)
        self.material_combo = QComboBox()
        self.material_combo.addItems(["钢材", "铝合金", "钛合金", "不锈钢", "铸铁", "自定义"])
        self.material_combo.currentTextChanged.connect(self.on_material_changed)
        material_layout.addWidget(self.material_combo, 0, 1)
        
        material_layout.addWidget(QLabel("弹性模量 (GPa):"), 1, 0)
        self.elastic_modulus = QDoubleSpinBox()
        self.elastic_modulus.setRange(1, 1000)
        self.elastic_modulus.setValue(210)
        self.elastic_modulus.setSuffix(" GPa")
        material_layout.addWidget(self.elastic_modulus, 1, 1)
        
        material_layout.addWidget(QLabel("泊松比:"), 2, 0)
        self.poisson_ratio = QDoubleSpinBox()
        self.poisson_ratio.setRange(0.1, 0.5)
        self.poisson_ratio.setValue(0.3)
        self.poisson_ratio.setDecimals(3)
        material_layout.addWidget(self.poisson_ratio, 2, 1)
        
        material_layout.addWidget(QLabel("密度 (kg/m³):"), 3, 0)
        self.density = QDoubleSpinBox()
        self.density.setRange(100, 20000)
        self.density.setValue(7850)
        self.density.setSuffix(" kg/m³")
        material_layout.addWidget(self.density, 3, 1)
        ...
        boundary_layout.addWidget(QLabel("约束类型:"), 0, 0)
        self.constraint_combo = QComboBox()
        self.constraint_combo.addItems(["固定底面", "固定侧面", "固定顶面", "简支", "自定义"])
        boundary_layout.addWidget(self.constraint_combo, 0, 1)
        
        boundary_layout.addWidget(QLabel("载荷类型:"), 1, 0)
        self.load_combo = QComboBox()
        self.load_combo.addItems(["重力", "压力", "集中力", "弯矩", "扭矩", "组合载荷"])
        boundary_layout.addWidget(self.load_combo, 1, 1)
        
        boundary_layout.addWidget(QLabel("载荷大小:"), 2, 0)
        self.load_magnitude = QDoubleSpinBox()
        self.load_magnitude.setRange(0.1, 10000)
        self.load_magnitude.setValue(1000)
        self.load_magnitude.setSuffix(" N")
        boundary_layout.addWidget(self.load_magnitude, 2, 1)
        ...
        analysis_layout.addWidget(QLabel("网格密度:"), 0, 0)
        self.mesh_density = QComboBox()
        self.mesh_density.addItems(["粗糙", "中等", "精细", "超精细"])
        self.mesh_density.setCurrentText("中等")
        analysis_layout.addWidget(self.mesh_density, 0, 1)
        
        analysis_layout.addWidget(QLabel("分析类型:"), 1, 0)
        self.analysis_type = QComboBox()
        self.analysis_type.addItems(["静力分析", "模态分析", "热应力分析", "疲劳分析"])
        analysis_layout.addWidget(self.analysis_type, 1, 1)
        
        analysis_layout.addWidget(QLabel("安全系数:"), 2, 0)
        self.safety_factor = QDoubleSpinBox()
        self.safety_factor.setRange(1.0, 10.0)
        self.safety_factor.setValue(2.0)
        self.safety_factor.setDecimals(2)
        analysis_layout.addWidget(self.safety_factor, 2, 1)
        
        layout.addWidget(analysis_group)
        
        # 按钮
        button_layout = QHBoxLayout()
        self.ok_button = QPushButton("开始分析")
        ...
        # 初始化材料属性
        self.on_material_changed("钢材")
        
    def on_material_changed(self, material):
        """材料改变时更新属性"""
        material_props = {
            "钢材": {"elastic": 210, "poisson": 0.3, "density": 7850},
            "铝合金": {"elastic": 70, "poisson": 0.33, "density": 2700},
            "钛合金": {"elastic": 116, "poisson": 0.32, "density": 4500},
            "不锈钢": {"elastic": 200, "poisson": 0.29, "density": 8000},
            "铸铁": {"elastic": 100, "poisson": 0.25, "density": 7200}
        }
        
        if material in material_props:
            props = material_props[material]
            self.elastic_modulus.setValue(props["elastic"])
            self.poisson_ratio.setValue(props["poisson"])
            self.density.setValue(props["density"])
    ...
    def get_analysis_params(self):
        ...
        return {
            ...
            "load_type": self.load_combo.currentText(),
            "load_magnitude": self.load_magnitude.value(),
            "mesh_density": self.mesh_density.currentText(),
...
class StressAnalysis:
    ...
    def calculate_stress(self, params):
        """计算应力分布"""
        if self.mesh is None:
            return None
        ...
        try:
            ...
            # 简化的有限元分析
            # 1. 计算单元刚度矩阵
            E = params["elastic_modulus"] * 1e9  # 转换为Pa
            nu = params["poisson_ratio"]
            
            # 2. 应用边界条件
            # 根据约束类型确定固定节点
            fixed_nodes = self._get_fixed_nodes(params["constraint"], vertices)
            
            # 3. 应用载荷
            # 根据载荷类型计算节点力
            node_forces = self._calculate_node_forces(params["load_type"], 
                                                    params["load_magnitude"], 
                                                    vertices, faces)
            
            # 4. 简化的应力计算（基于几何和材料属性）
            stress_field = self._calculate_simplified_stress(vertices, faces, 
                                                           E, nu, node_forces, fixed_nodes)
            
            # 5. 计算位移
            displacement_field = self._calculate_displacement(vertices, stress_field, E, nu)
            
            return {
                "stress": stress_field,
                "displacement": displacement_field,
                "von_mises": self._calculate_von_mises_stress(stress_field),
                "safety_factor": self._calculate_safety_factor(stress_field, params["safety_factor"])
            }
    ...
    def _get_fixed_nodes(self, constraint_type, vertices):
        """获取固定节点"""
        if constraint_type == "固定底面":
            # 固定Z坐标最小的节点
            min_z = np.min(vertices[:, 2])
            return np.where(vertices[:, 2] <= min_z + 0.1)[0]
        elif constraint_type == "固定侧面":
            # 固定X坐标最小和最大的节点
            min_x, max_x = np.min(vertices[:, 0]), np.max(vertices[:, 0])
            return np.where((vertices[:, 0] <= min_x + 0.1) | (vertices[:, 0] >= max_x - 0.1))[0]
        elif constraint_type == "固定顶面":
            # 固定Z坐标最大的节点
            max_z = np.max(vertices[:, 2])
            return np.where(vertices[:, 2] >= max_z - 0.1)[0]
        else:
            # 默认固定底面
            min_z = np.min(vertices[:, 2])
            return np.where(vertices[:, 2] <= min_z + 0.1)[0]
    
    def _calculate_node_forces(self, load_type, magnitude, vertices, faces):
        """计算节点力"""
        num_vertices = len(vertices)
        node_forces = np.zeros((num_vertices, 3))
        
        if load_type == "重力":
            # 重力载荷
            g = 9.81  # m/s²
            for i in range(num_vertices):
                node_forces[i, 2] = -magnitude * g  # Z方向重力
            ...
            for face in faces:
                ...
                # 分配力到顶点
                force_per_vertex = magnitude * area / 3
                for vertex_id in face:
                    node_forces[vertex_id] += force_per_vertex * normal
                    
        elif load_type == "集中力":
            # 集中力（作用在最高点）
            max_z_idx = np.argmax(vertices[:, 2])
            node_forces[max_z_idx, 2] = -magnitude
            
        return node_forces
    
    def _calculate_simplified_stress(self, vertices, faces, E, nu, node_forces, fixed_nodes):
        """简化的应力计算"""
        num_vertices = len(vertices)
        stress_field = np.zeros((num_vertices, 6))  # [σxx, σyy, σzz, τxy, τyz, τxz]
        ...
        for i in range(num_vertices):
            ...
            # 简化的应力公式
            stress_field[i, 0] = force[0] / (E * (1 - nu**2))  # σxx
            stress_field[i, 1] = force[1] / (E * (1 - nu**2))  # σyy
            stress_field[i, 2] = force[2] / (E * (1 - nu**2))  # σzz
            stress_field[i, 3] = 0.5 * (force[0] + force[1]) / (E * (1 + nu))  # τxy
            stress_field[i, 4] = 0.5 * (force[1] + force[2]) / (E * (1 + nu))  # τyz
            stress_field[i, 5] = 0.5 * (force[0] + force[2]) / (E * (1 + nu))  # τxz
            
        return stress_field
    
    def _calculate_displacement(self, vertices, stress_field, E, nu):
        """计算位移"""
        num_vertices = len(vertices)
        displacement = np.zeros((num_vertices, 3))
        
        for i in range(num_vertices):
            stress = stress_field[i]
            # 简化的位移计算
            displacement[i, 0] = stress[0] * (1 - nu) / E
            displacement[i, 1] = stress[1] * (1 - nu) / E
            displacement[i, 2] = stress[2] * (1 - nu) / E
            
        return displacement
    
    def _calculate_von_mises_stress(self, stress_field):
        """计算von Mises应力"""
        von_mises = np.zeros(len(stress_field))
    ...
    def _calculate_safety_factor(self, stress_field, target_safety_factor):
        """计算安全系数"""
        von_mises = self._calculate_von_mises_stress(stress_field)
        ...
        if max_stress > 0:
            ...
            return min(actual_safety_factor, target_safety_factor)
        else:
            return target_safety_factor
    ...
    def show_ribbon(self, idx):
        ...
        if idx == 0:
            ...
            btns = [
                ...
                ("⚡", "受力分析", self.on_stress_analysis, "受力分析与应力分布")
            ]
        elif idx == 2:
            btns = [
            ...
            btns = [
                ...
                ("⚙️", "ABB参数", self.on_abb_params, "配置ABB机械臂参数"),
...
ENDMODULE"""
    ...
    # ==================== 机器人控制功能 ====================
    def on_abb_params(self):
        """打开ABB参数配置对话框"""
        try:
            dialog = ABBParamDialog(self)
            if dialog.exec_() == dialog.Accepted:
                params = dialog.get_params()
                self.abb_params = params
                self.status_left.setText("状态：ABB参数配置已保存")
                # 可以在这里添加参数应用到系统的逻辑
            else:
                self.status_left.setText("状态：取消ABB参数配置")
        except Exception as e:
            QMessageBox.warning(self, "错误", f"打开ABB参数配置失败：{str(e)}")
            self.status_left.setText(f"状态：ABB参数配置失败 - {str(e)}")
...
• 基础3D预览功能"""
    ...
    def _init_statusbar(self):
        ...
        # 初始化受力分析相关变量
        self.stress_analysis = StressAnalysis()
        self.stress_results = None
        self.stress_actor = None

    # ==================== 受力分析功能 ====================
    def on_stress_analysis(self):
        """受力分析功能"""
        try:
            if not self.model_selected or not hasattr(self, 'current_mesh'):
            ...
            # 打开受力分析参数设置对话框
            dialog = StressAnalysisDialog(self)
            if dialog.exec_() == dialog.Accepted:
                params = dialog.get_analysis_params()
                self.perform_stress_analysis(params)
            else:
                self.status_left.setText("状态：取消受力分析")
                
        except Exception as e:
            QMessageBox.warning(self, "错误", f"受力分析失败：{str(e)}")
            self.status_left.setText(f"状态：受力分析失败 - {str(e)}")
    
    def perform_stress_analysis(self, params):
        """执行受力分析"""
        try:
            self.status_left.setText("状态：正在进行受力分析...")
            QApplication.processEvents()
            
            # 设置网格
            self.stress_analysis.set_mesh(self.current_mesh)
            
            # 计算应力
            results = self.stress_analysis.calculate_stress(params)
            
            if results is None:
                QMessageBox.warning(self, "错误", "应力计算失败！")
                self.status_left.setText("状态：应力计算失败")
                return
            ...
            # 显示应力分布
            self.display_stress_results(results, params)
            
            # 显示分析报告
            self.show_stress_report(results, params)
            
            self.status_left.setText("状态：受力分析完成")
            
        except Exception as e:
            QMessageBox.warning(self, "错误", f"受力分析执行失败：{str(e)}")
            self.status_left.setText(f"状态：受力分析失败 - {str(e)}")
    
    def display_stress_results(self, results, params):
        """显示应力分布结果"""
        try:
            import numpy as np
            
            # 清除之前的应力显示
            if self.stress_actor is not None:
                self.pv_widget.remove_actor(self.stress_actor)
            ...
            if stress_max > stress_min:
                ...
                normalized_stress = np.zeros_like(von_mises)
            ...
            # 添加应力数据
            pv_mesh.point_data["von_mises"] = von_mises
            pv_mesh.point_data["normalized_stress"] = normalized_stress
            
            # 显示应力分布
            self.stress_actor = self.pv_widget.add_mesh(
                pv_mesh,
                scalars="von_mises",
                cmap="jet",  # 使用jet颜色映射
                show_edges=True,
    ...
    def show_stress_report(self, results, params):
        """显示应力分析报告"""
        try:
            # 创建报告对话框
            dialog = QDialog(self)
            dialog.setWindowTitle("受力分析报告")
            dialog.setFixedSize(600, 500)
            ...
            # 生成报告内容
            report_content = self.generate_stress_report(results, params)
            ...
            export_btn.clicked.connect(lambda: self.export_stress_report(report_content))
    ...
    def generate_stress_report(self, results, params):
        """生成应力分析报告"""
        try:
            import numpy as np
...
1. 分析参数
{'-'*20}
材料类型: {params['material']}
弹性模量: {params['elastic_modulus']} GPa
泊松比: {params['poisson_ratio']}
密度: {params['density']} kg/m³
约束类型: {params['constraint']}
载荷类型: {params['load_type']}
载荷大小: {params['load_magnitude']} N
分析类型: {params['analysis_type']}
安全系数: {params['safety_factor']}

2. 应力分析结果
{'-'*20}
最大Von Mises应力: {np.max(von_mises):.2e} Pa
最小Von Mises应力: {np.min(von_mises):.2e} Pa
平均Von Mises应力: {np.mean(von_mises):.2e} Pa
应力标准差: {np.std(von_mises):.2e} Pa

3. 位移分析结果
{'-'*20}
最大位移: {np.max(np.linalg.norm(displacement, axis=1)):.6f} m
平均位移: {np.mean(np.linalg.norm(displacement, axis=1)):.6f} m

4. 安全评估
{'-'*20}
计算安全系数: {safety_factor:.2f}
安全状态: {'安全' if safety_factor >= params['safety_factor'] else '不安全'}

5. 建议
{'-'*20}
"""
            
            if safety_factor >= params['safety_factor']:
                report += "✓ 结构在当前载荷条件下是安全的\n"
                report += "✓ 可以继续使用当前设计\n"
            else:
                report += "⚠ 结构安全系数不足\n"
                report += "建议:\n"
    ...
    def export_stress_report(self, report_content):
        """导出应力分析报告"""
        try:
            file_path, _ = QFileDialog.getSaveFileName(
                self, "保存应力分析报告", 
                f"应力分析报告_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                "文本文件 (*.txt);;所有文件 (*.*)"
            )
            ...
            if file_path:
                ...
                self.status_left.setText(f"状态：应力分析报告已导出")
            else:
                self.status_left.setText("状态：取消导出报告")
                
        except Exception as e:
            QMessageBox.warning(self, "错误", f"导出报告失败：{str(e)}")
            self.status_left.setText(f"状态：导出报告失败 - {str(e)}")
