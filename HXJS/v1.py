...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                            ...
                            QDoubleSpinBox, QSpinBox)  # 新增QDoubleSpinBox和QSpinBox
...
class HeatMap3D(CustomFigureCanvas):
    """三维热力图，展示电压-送丝速度-质量关系，增强版"""
    def __init__(self, *args, **kwargs):
        super(HeatMap3D, self).__init__(*args, **kwargs)
        self.axes = self.fig.add_subplot(111, projection='3d')
        self.axes.set_title('参数-质量关系热力图', color='#1E1E1E')
        self.axes.set_xlabel('电压 (V)', color='#1E1E1E')
        self.axes.set_ylabel('送丝速度 (cm/min)', color='#1E1E1E')
        self.axes.set_zlabel('质量评分', color='#1E1E1E')
        self.axes.tick_params(axis='x', colors='#1E1E1E')
        self.axes.tick_params(axis='y', colors='#1E1E1E')
        self.axes.tick_params(axis='z', colors='#1E1E1E')
        self.voltage_data = []
        self.speed_data = []
        self.quality_data = []
        self.v_range = np.linspace(180, 260, 20)
        self.s_range = np.linspace(100, 300, 20)
        self.V, self.S = np.meshgrid(self.v_range, self.s_range)
        self.Q = np.zeros_like(self.V)
        self.custom_cmap = cm.colors.LinearSegmentedColormap.from_list(
            'quality_cmap', [(1, 0, 0), (1, 0.5, 0), (0, 1, 0)], N=256
        )
        self.surf = None  # 初始无曲面
        self.scatter = self.axes.scatter([], [], [], c='white', marker='o', s=80, edgecolors='black', linewidth=1)
        self.axes.view_init(elev=30, azim=45)
        self.axes.set_facecolor('#1E1E1E')
        self.fig.patch.set_facecolor('#1E1E1E')
        self.axes.grid(True, linestyle='--', alpha=0.3, color='#1E1E1E')
        self._colorbar = None
        # 只添加一次颜色条
        self.surf = self.axes.plot_surface(self.V, self.S, self.Q, cmap=self.custom_cmap, linewidth=0, antialiased=True, alpha=0.7)
        self._colorbar = self.fig.colorbar(self.surf, ax=self.axes, shrink=0.5, aspect=5)
        self._colorbar.set_label('质量评分', color='#1E1E1E')
        self._colorbar.ax.yaxis.set_tick_params(color='#1E1E1E')
        self._colorbar.outline.set_edgecolor('#1E1E1E')
        plt.setp(plt.getp(self._colorbar.ax.axes, 'yticklabels'), color='#1E1E1E')
        self._colorbar.ax.set_facecolor('#1E1E1E')
        for spine in self._colorbar.ax.spines.values():
            spine.set_edgecolor('#1E1E1E')

    def update_data(self, voltage, speed, quality):
        """更新热力图数据（优化：刷新前清理旧曲面，数据点不足3个时只显示散点，颜色条只添加一次）"""
        ...
        if self.surf is not None:
            ...
            self.surf = None
        # 更新散点
        colors = [self.custom_cmap(q/100) for q in self.quality_data]
        self.scatter.remove()
        self.scatter = self.axes.scatter(self.voltage_data, self.speed_data, self.quality_data, c=colors, marker='o', s=80, edgecolors='black', linewidth=1)
        # 曲面插值
        if len(self.voltage_data) >= 3:
            from scipy.interpolate import griddata
            points = np.column_stack((self.voltage_data, self.speed_data))
            values = np.array(self.quality_data)
            try:
                self.Q = griddata(points, values, (self.V, self.S), method='cubic', fill_value=min(values) if values.size > 0 else 0)
                self.surf = self.axes.plot_surface(self.V, self.S, self.Q, cmap=self.custom_cmap, linewidth=0, antialiased=True, alpha=0.7)
            except Exception as e:
                print(f"更新热力图时出错: {e}")
        # 坐标轴范围
        if self.voltage_data:
            self.axes.set_xlim(min(self.voltage_data)-10, max(self.voltage_data)+10)
        if self.speed_data:
            self.axes.set_ylim(min(self.speed_data)-10, max(self.speed_data)+10)
        if self.quality_data:
            self.axes.set_zlim(min(self.quality_data)-5, max(self.quality_data)+5)
        self.draw()
...
class QualityMeter(FigureCanvas):
    """质量仪表盘"""
    def __init__(self, parent=None, width=4, height=4, dpi=100):
        self.fig = Figure(figsize=(width, height), dpi=dpi)
        self.axes = self.fig.add_subplot(111, polar=True)
        super(QualityMeter, self).__init__(self.fig)
        self.setParent(parent)
        ...
        self.radii = np.ones_like(self.angles) * 1.0
        
        # 创建颜色映射
        self.colors = []
        for angle in self.angles:
            # 角度映射到质量分数（0-100）
            quality = (angle / np.pi) * 100
            
            # 颜色映射（红色-橙色-绿色）
            if quality < 70:
                self.colors.append((1, 0, 0, 0.7))  # 红色
            elif quality < 85:
                self.colors.append((1, 0.5, 0, 0.7))  # 橙色
            else:
                self.colors.append((0, 1, 0, 0.7))  # 绿色
        
        # 绘制颜色区域
        self.axes.bar(self.angles, self.radii, width=np.pi/100, bottom=0.0, color=self.colors)
        
        # 绘制刻度线
        for i in range(0, 101, 10):
            angle = (i / 100) * np.pi
            self.axes.plot([angle, angle], [0.9, 1.0], color='#CCCCCC', linewidth=1)
            self.axes.text(angle, 0.8, f"{i}", ha='center', va='center', color='#CCCCCC')
        
        # 绘制中心圆
        self.center_circle = plt.Circle((0, 0), 0.6, 
                                      transform=self.axes.transData,  # 修正变换参数
                                      facecolor='#1E1E1E', edgecolor='#4A4A4A', 
                                      linewidth=2, zorder=10)
        self.axes.add_artist(self.center_circle)
        
        # 初始化指针
        self.quality = 50
        self.pointer, = self.axes.plot([0, np.pi * self.quality / 100], [0, 0.5], 
                                      color='#FF9900', linewidth=3, marker='o', markersize=8, 
                                      markerfacecolor='#FF9900', markeredgecolor='#FFFFFF')
        
        # 添加标题
        self.axes.set_title("质量评分", color='#FFFFFF', pad=30)
        
        # 添加当前值标签
        self.value_text = self.axes.text(0, 0.4, f"{self.quality}", ha='center', va='center', 
    ...
    def update_quality(self, quality):
        ...
        # 更新指针
        angle = np.pi * self.quality / 100 + np.pi * 0.5  # 添加90度偏移
        self.pointer.set_xdata([0, angle])
        
        # 更新值标签
        self.value_text.set_text(f"{self.quality:.1f}")
        ...
        self.pointer.set_color(color)
        self.pointer.set_markerfacecolor(color)
...
class IndustrialStyle(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 连接按钮点击事件
        self.start_button.clicked.connect(lambda: (self.start_welding(), self.append_log("点击开始焊接")))
        self.stop_button.clicked.connect(lambda: (self.stop_welding(), self.append_log("点击停止焊接")))
        self.optimize_button.clicked.connect(lambda: (self.optimize_parameters(), self.append_log("点击参数优化")))
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, 'distribution_chart1'):
                    self.distribution_chart1.update_data(220, 85, 85)
                if hasattr(self, 'distribution_chart2'):
                    self.distribution_chart2.update_data(220, 85, 85)
            except Exception as e:
...
class PathSlicerTab(QWidget):
    def __init__(self, parent=None):
        ...
        # 左侧3D预览
        self.fig = Figure(figsize=(8,9), dpi=100, tight_layout=True)
        self.canvas = FigureCanvas(self.fig)
        op_layout.addWidget(self.canvas, 2)
        # 右侧参数与过程
        right_panel = QWidget()
    ...
    def show_empty_preview(self):
        ...
        ax.text(0.5,0.5,0.5,"请导入STL模型",color='gray')
        self.canvas.draw()
    ...
    def preview_mesh(self):
        ...
        if self.mesh is not None and hasattr(self.mesh, 'vertices') and hasattr(self.mesh, 'faces'):
            ...
            try:
                ...
                shade = np.clip(np.dot(face_normals, light_dir), 0.2, 1.0)
                face_colors = np.zeros((len(f), 4))
                face_colors[:,:3] = 0.6 * shade[:,None]
                face_colors[:,3] = 0.85
                mesh_collection = Poly3DCollection(v[f], facecolors=face_colors, edgecolors=(0.3,0.3,0.3,0.3), linewidths=0.2)
                ...
                for axis, m in enumerate(mid):
                    getattr(ax, f'set_{"xyz"[axis]}lim')(m - max_range, m + max_range)
                # 坐标轴长度=模型最大边长
                axis_len = max_range * 2
                # 绘制坐标轴（不影响缩放）
                ax.quiver(mid[0], mid[1], mid[2], axis_len, 0, 0, color='r', linewidth=3)
                ax.quiver(mid[0], mid[1], mid[2], 0, axis_len, 0, color='g', linewidth=3)
                ax.quiver(mid[0], mid[1], mid[2], 0, 0, axis_len, color='b', linewidth=3)
                ax.text(mid[0]+axis_len, mid[1], mid[2], 'X', color='r', fontsize=16, fontweight='bold')
                ax.text(mid[0], mid[1]+axis_len, mid[2], 'Y', color='g', fontsize=16, fontweight='bold')
                ax.text(mid[0], mid[1], mid[2]+axis_len, 'Z', color='b', fontsize=16, fontweight='bold')
                ax.set_title("STL模型预览（可调方向/缩放/坐标轴自适应）")
                ax.grid(False)
                ax.set_axis_off()
            except Exception as e:
                ax.text(0.5,0.5,0.5,f"预览失败\n{e}",color='r')
                self.append_log(f"模型预览失败: {e}")
        else:
            ax.text(0.5,0.5,0.5,"无模型数据",color='gray')
        self.canvas.draw()
    def run_slicer(self):
        ...
        if self.mesh is not None and hasattr(self.mesh, 'vertices') and hasattr(self.mesh, 'faces'):
            try:
                ...
                max_range = (xyz.max(axis=0) - xyz.min(axis=0)).max() / 2.0
                mid = xyz.mean(axis=0)
                axis_len = max_range * 0.4
                for axis, m in enumerate(mid):
                    getattr(ax, f'set_{"xyz"[axis]}lim')(m - max_range, m + max_range)
                ax.quiver(mid[0], mid[1], mid[2], axis_len, 0, 0, color='r', linewidth=2)
                ax.quiver(mid[0], mid[1], mid[2], 0, axis_len, 0, color='g', linewidth=2)
                ax.quiver(mid[0], mid[1], mid[2], 0, 0, axis_len, color='b', linewidth=2)
                ax.text(mid[0]+axis_len, mid[1], mid[2], 'X', color='r', fontsize=12)
                ax.text(mid[0], mid[1]+axis_len, mid[2], 'Y', color='g', fontsize=12)
                ax.text(mid[0], mid[1], mid[2]+axis_len, 'Z', color='b', fontsize=12)
                ax.grid(False)
                ax.set_axis_off()
            except Exception:
                pass
        ...
        if path is not None and path.shape[0] > 0:
            ax.scatter(path[:,0], path[:,1], path[:,2], c=path[:,2], cmap='viridis', s=8, alpha=0.9, edgecolors='none')
            ax.set_title("切片路径点云预览")
            self.append_log(f"切片成功，生成路径点数: {path.shape[0]}")
            self.tabs.setCurrentIndex(0)
        else:
            ax.set_title("无有效路径，仅显示模型")
            # 自动分析原因
            reason = []
            if self.mesh is not None and hasattr(self.mesh, 'bounds'):
                zmin, zmax = self.mesh.bounds[0][2], self.mesh.bounds[1][2]
                total_h = zmax-zmin
                if total_h < 0.5:
                    reason.append("模型Z高度过小")
                if self.layer_spin.value() > total_h:
                    reason.append("层高大于模型高度")
            if self.grid_spin.value() > 2.0:
                reason.append("网格间距过大")
            ...
            if hasattr(self.mesh, 'split'):
                ...
                areas = [getattr(c, 'area', 0) for c in components]
                if all(a < self.min_area_spin.value() for a in areas):
                    reason.append("所有区域面积小于最小缺陷面积")
            if not reason:
                reason.append("模型缺陷区域过小或未闭合/参数不适配")
            self.append_log("<font color='red'>切片无有效路径，请检查模型或参数！\n可能原因：" + ", ".join(reason) + "</font>")
            self.tabs.setCurrentIndex(1)
        self.canvas.draw()
